...
describe('Generate Logo', () => {
    ...
    describe('Logo Test', () => {
        it('should return all selections as an svg file', () => {
            ...
            const text = `<text x="150" y="130" text-anchor="middle" fill="blue" font-size="64"> cat </text>`;
            let svg = circle.render();
            const newLogo = `${svgString} ${svg} ${text} </svg>`;
            expect(newLogo).toEqual(`<svg version="1.1" width="300" height="200" xmlns="http://www.w3.org/2000/svg"> <circle cx="150" cy="115" r="80" fill="red" /> <text x="150" y="130" text-anchor="middle" fill="blue" font-size="64"> cat </text> </svg>`)
        })
    })
});